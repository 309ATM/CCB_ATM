...
public class ConnectTable{
	...
	public String[] getUserMessage(long card){
		...
		//获取id
		int id = BankAccout.getInstance().getAccountDAO().getAccountId(card);
		//获取用户消息,用id查询
		userRecords = BankAccout.getInstance().getUserDao().getUserMessageId(id);
		//获取账户状态和类型,用卡号查询
		accountRecords = BankAccout.getInstance().getAccountDAO().getAccountTypeStatus(card);
